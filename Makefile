...
.PHONY: manifests
manifests: controller-gen ## Generate RBAC manifests (CRDs are generated via Python).
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name kb-init-builder
	$(CONTAINER_TOOL) buildx use kb-init-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm kb-init-builder
	rm Dockerfile.cross
...
uninstall: ## Uninstall CRDs from the K8s cluster specified in ~/.kube/config. Call with ignore-not-found=true to ignore resource not found errors during deletion.
	$(KUBECTL) delete --ignore-not-found=$(ignore-not-found) -f config/crd/crds.yaml
