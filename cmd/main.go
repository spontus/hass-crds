...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "448380ef.home-assistant.io",
	...
	mqttConfig, err := mqtt.NewConfigFromEnv()
	if err != nil {
		setupLog.Error(err, "unable to load MQTT configuration")
		os.Exit(1)
	}

	mqttClient := mqtt.NewClient(mqttConfig, setupLog)

	// Connect to MQTT broker
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	if err := mqttClient.Connect(ctx); err != nil {
		setupLog.Error(err, "unable to connect to MQTT broker")
		os.Exit(1)
	}
